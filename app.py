...
class tkinterApp(tk.Tk):
	...
	def __init__(self, *args, **kwargs):
		...
		for F in (StartPage,GestionVoiture,GestionFournisseur,GestionSupprimer,GestionAjouter,GestionModifier,GestionChercher,GestionAjouterVoiture,GestionModifierVoiture,GestionSupprimerVoiture,GestionChercherVoiture,ExproterFournisseur,Visualisation,ExproterVoiture):
...
class StartPage(tk.Frame):
        def __init__(self, parent, controller):
            ...
            # putting the button in its place by
            # using grid
            button4 = ttk.Button(self, text ="Visualisation", style="TButton",command = home)
            ...
            btexit=ttk.Button(self, text="Gestion Location", style="TButton",command= index)
            btexit.grid(row=5,column=1,ipady = 3,ipadx = 13,pady = 40)
        def quitter(self):  
...
class GestionVoiture(tk.Frame):
	...
	def __init__(self, parent, controller):
            ...
            btn_search.grid(row = 4,column = 0,ipady = 4,ipadx = 13,pady = 40)
             #======================boutton exporter 
            btn_exporter= ttk.Button(self, text="Exporter",command= lambda : controller.show_frame(ExproterVoiture)) 
            ...
            btn_exit.grid(row = 6,column = 0,ipady = 4,ipadx = 13,pady = 40)
...
class ExproterFournisseur(tk.Frame):
    ...
    def exit(self):  
        ...
        if MsgBox == 'yes':  
            Frame.destroy(self)  
class ExproterVoiture(tk.Frame):
    def __init__(self,parent, controller): 
        tk.Frame.__init__(self, parent,bg="lightblue")
        bg = ImageTk.PhotoImage(file = "v3.png")
        bg_label = tk.Label(self, image = bg)
        ...
        def save_csv(rows):
            header=["matricule","modele","typeVoiture","nbrePlace","prix","quantite"]
            
            with open('voiture.csv','w') as f:
                w=csv.writer(f,dialect="excel")
                w.writerow(header)
                w.writerows(rows)
